...
import com.csyd.core.util.MenuNode;
...
public class MenuController {
  ...
  public void index(HttpServletRequest request, HttpServletResponse response) {
    ...
    List<Integer> list = menuService.loadUserMenus(user.getUserId());
    ...
    for (SysMenu menu : menus) {
      if (list.contains(menu.getMenuId())) {// 判断是否存在
   ...
   * @return
   */
  public MenuNode convertMenuToNode(SysMenu menu) {
    MenuNode node = new MenuNode();
    node.setId(menu.getMenuId());
    node.setText(menu.getMenuName());
    node.setState("closed");

    List<SysMenu> menus = menuService.loadChileMenus(menu.getMenuId());
    if (menus.size() == 0)
      node.setState("open");
    if (menu.getMenuParentId() != null)
      node.setParentId(menu.getMenuParentId());
    node.setUrl(menu.getMenuLinkUrl());
    return node;
  }
  ...
  public void list(@RequestParam(required = true, value = "page") Integer page,
      ...
      @RequestParam(required = false, value = "menuDescn") String menuDescn,
      @RequestParam(required = false, value = "menuParentId") Integer menuParentId, HttpServletResponse response) {

    if (!StringUtils.isEmpty(menuName)) {
      menuName = "%" + menuName + "%";
    }
    if (!StringUtils.isEmpty(menuDescn)) {
      menuDescn = "%" + menuDescn + "%";
    }
    ...
    Pager<SysMenu> pager = menuService.findPager(pageno, pagesize, sort, order, menuName, menuDescn, menuParentId);
  ...
  private void loadComboTree(String title, HttpServletRequest request, HttpServletResponse response) {
    ...
    if (request.getParameter("id") != null && !"".equals(request.getParameter("id"))) {
      ...
      if (title != null) {
        ...
        SysMenu topAllMenu = new SysMenu();
        topAllMenu.setMenuId(0);
        topAllMenu.setMenuName(title);
  ...
  public String view(@RequestParam(required = true, value = "id") Integer id, ModelMap modelMap) {
    if (id != null) {
      ...
      // 显示父级菜单
      if (menu.getMenuParentId() != null) {
        SysMenu parentMenu = menuService.findById(menu.getMenuParentId());
  ...
  public void save(SysMenu menu, HttpServletResponse response) {
    int count = 0;

    if (menu.getMenuParentId() == null || menu.getMenuParentId() == 0) {
      menu.setMenuParentId(null); // 无父级菜单
    }
